...
function_spatial_regression_analyses <- "SPatial_analysis_spatial_reg_03222015_functions.R" #PARAM 1
...
n_time_event <- 14 #PARAM 15 #this is the timestep corresponding to the event ie Hurricane Katrina (1992-20111 here)
...
plot(subset(s_raster,zonal_colnames))
...
lines(1:length(var_names),pix200,type="b",ylab="var",xlab="time step",col=c("red"),
     main="Average variable and pixel 200 profile")
legend("bottomleft",legend=c("Overall average VAR ","VAR 200 "),
...
out_suffix_s <- paste("t_",2:length(time_window_selected),"_",out_suffix,sep="")#this should really be automated!!!
estimator <- "lm"
...
num_cores_tmp <- 11
time_step <- n_time_event -1 #this is the time step for which to start the arima model with
n_pred_ahead <- 4
