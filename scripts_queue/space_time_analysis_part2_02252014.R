...
list_param_predict_arima_2 <- list(pix_val=pix_val,na.rm=T,arima_order=NULL,n_ahead=4)
...
out_rastnames <- paste(c("test_mooore1_auto","test_moore2_auto"),"_",out_suffix,file_format,sep="")
...
r_pred_t1 <- raster(pred_t1[[1]]) #timestep 1
r_pred_t2 <- raster(pred_t2[[1]]) #timestep 2
r_error_t1 <- raster(pred_t1[[2]]) #timestep 1, error in fitting model
r_error_t2 <- raster(pred_t2[[2]]) #timestep 2
...
list_param_predict_arima_3 <- list(pix_val=pix_val,na.rm=T,arima_order=c(1,0,1),n_ahead=2)
