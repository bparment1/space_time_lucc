...
function_spatial_regression_analyses <- "SPatial_analysis_spatial_reg_03182015_functions.R" #PARAM 1
...
proj_str<- CRS_WGS84 
CRS_interp <- proj_modis_str # PARAM 4

file_format <- ".rst" #PARAM5
NA_value <- -9999 #PARAM6
NA_flag_val <- NA_value #PARAM7
out_suffix <-"light_Katrina_03222015" #output suffix for the files and ouptu folder #PARAM 8
...
zonal_colnames <- "high1_lowminus1" #PARAM 12
var_names <- 3:22 #PARAM 13 #Data is stored in the columns 3 to 22
num_cores <- 11 #PARAM 14
time_event <- 14 #PARAM 15 #this is the timestep corresponding to the event ie Hurricane Katrina (1992-20111 here)
...
dim(r_stack) #21x28x27 
ncell(s_raster) #588
freq(r_FID,value=NA) #338
ncell(s_raster) - freq(r_FID,value=NA) #250
...
plot(r_stack,y=1:4)
histogram(subset(r_stack,1:4))

mean_vals <- colMeans(data_tb[,var_names],na.rm=T)
pix200 <- data_tb[200,var_names]
pix300 <- data_tb[300,var_names]

plot(1:length(var_names),mean_vals,type="b",ylab="var",xlab="time step",
     main="Average variable and pixel 200 profile")
lines(1:length(var_names),pix200,type="b",ylab="var",xlab="time step",col=c("red"),
     main="Average variable and pixel 200 profile")
legend("bottomleft",legend=c("Overall average pop ","Pixel 200 pop"),
...
r_var <- subset(r_stack,13:14) #date before hurricane is 153
#r_clip <- rast_ref
...
r_var <- subset(r_stack,14) #this is the date we want to use to run the spatial regression
...
##############################################################################################
############## PART V PREDICT MODELS FOR USING TEMP AND SPAT REGRESSION OVER a range of time steps ####

##This times will we use an automated function to generate predictions over a range of time step dates
...
out_suffix_s <- paste("t_",var_names,out_suffix,sep="") #use mle estimation, name "mle" is included in the output automatically?
estimator <- "mle"
