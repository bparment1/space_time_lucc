...
out_suffix_s <- paste(c("_LM_OLS_.*"),out_suffix,sep="_")
...
########### Combining all accuracy: spat_reg, reg_ols and arima for four dates...

#accuracy combined by the total area of study: overall MAE

mae_tot_tb1 <- read.table("mae_tot_tb_03112014.txt")
mae_tot_tb2 <- read.table("mae_tot_tb_03132014.txt")

#mae_tot_tb2$method <- c("ols_reg","ols_reg","ols_reg","arima","arima","arima")
mae_tot_tb_combined <- cbind(mae_tot_tb1[,1],mae_tot_tb2)
names(mae_tot_tb_combined) <- c("spat_reg","ols_reg","arima","time")
...
lines(arima ~ time, type="b",col="cyan",pch=3,data=mae_tot_tb_combined)
#axis(side=1,las=1,tick=TRUE,
#       at=breaks_lab,labels=tick_lab, cex.axis=1.2,font=2) #reduce number of labels to Jan and June
#  #text(tick_lab, par(\u201cusr\u201d)[3], labels = tick_lab, srt = 45, adj = c(1.1,1.1), xpd = TRUE, cex=.9)
#  axis(2,cex.axis=1.4,font=2)
#  box()
legend("topleft",legend=names(mae_tot_tb_combined)[1:3], 
        cex=1.4, col=c("magenta","green","cyan"),bty="n",
        lty=1,pch=1:3)
title("Average Mean Absolute Error (MAE) by method",cex=1.6, font=2)
...
mae_zones_tb_combinded <- rbind(mae_zones_tb1[1:3,],mae_zones_tb2)

write.table(mae_zones_tb_combinded,file=paste("mae_zones_tb_combined","_",out_suffix,".txt",sep=""))

mae_val <- (as.vector(as.matrix(mae_zones_tb_combinded[,2:5])))
avg_ac_tb <- as.data.frame(mae_val)

avg_ac_tb$metric <- rep("mae",length(mae_val))
avg_ac_tb$zones <- rep(c(3,4,5),4)
avg_ac_tb$time <- c(rep(1,9),rep(2,9),rep(3,9),rep(4,9))
