...
raster_ts_arima_predict <- function(i,list_param){
  ...
  pred_obj <- list(y,y_error)
  names(pred_obj)<-c("pred","error")
  save(arima_mod,"arima_mod_",i,".RData")             
...
#chunek_fname <- "~/Data/Space_Time/ChunekFire_20110419_pol_sin.rst" #Fire 2
#chanchen_fname <- "~/Data/Space_Time/ChanchenFire_20110405_pol_sin.rst" #Fire 
...
#chanchen_sp <- readOGR(dsn=dirname(chanchen_fname),
#                     layer=gsub(extension(basename(chanchen_fname)),"",basename(chanchen_fname))) 
#chunek_sp <- spTransform(chunek_sp,CRS(CRS_interp))
#chunek_sp$fire_id <- 1:nrow(chunek_sp)
#chanchen_sp$fire_id <- 1:nrow(chanchen_sp)
#Convert polygon df to raster
#r_chunek <- rasterize(chunek_sp,dif_pred,"fire_id",fun=min)
#r_chanchen <- rasterize(chanchen_sp,dif_pred,"fire_id")
r_chanchen <- raster(chanchen_fname)

r_chunek <- raster(chunek_fname)

freq(r_chanchen) # 1 pixel
freq(r_chunek)   # 3 pixels

mae_chanchen <- zonal(abs(dif_pred),r_chanchen,fun="mean") #2011/04/05
mae_chunek <- zonal(abs(dif_pred),r_chunek,fun="mean")   #2011/04/19

#Chunchek Fire on April 19, 2011
day_event1 <-strftime(as.Date("2011.04.05",format="%Y.%m.%d"),"%j")#095
day_event2 <-strftime(as.Date("2011.04.19",format="%Y.%m.%d"),"%j")#109

#[237] "reg_mosaiced_MOD13A2_A2011097__005_1_km_16_days_NDVI_09242013_09242013" #after Chanchen
#[238] "reg_mosaiced_MOD13A2_A2011113__005_1_km_16_days_NDVI_09242013_09242013" #After Chunek
