...
out_suffix <-"04232014" #output suffix for the files that are masked for quality and for 
...
reg_var_list <- list.files(path=file.path(in_dir,"moore_NDVI_wgs84"),
                           pattern="moore_reg.*.MOD13A2_A.*04062014.*.tif$",full.name=TRUE)
...
##Subset spatially the data
r_stack_w <- crop(r_stack,rast_ref)

#### Now do Moore area!! about 3854 pixels
...
r_obj_name <- paste("raster_ts_arima_predict_obj",out_suffix,".RData",sep="")
save(ttx2,file=r_obj_name)
rm(ttx2)
ttx3 <- load_obj(r_obj_name)
