...
function_spatial_regression_analyses <- "SPatial_analysis_spatial_reg_08252014_functions.R"
...
out_suffix <-"EDGY_predictions_08252014" #output suffix for the files that are masked for quality and for 
...
for(i in 1:length(list_timesteps)){
  ...
  spat_mod_spreg_test <- try(spreg(v1 ~ var ,data=data_reg, listw= reg_listw_w, model="error",   
                     het = TRUE, verbose=TRUE))
